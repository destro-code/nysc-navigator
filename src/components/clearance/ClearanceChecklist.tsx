...
import { CheckSquare, Square, Download, Trophy, AlertCircle, FileText, Briefcase, Calendar, GraduationCap, Loader2, Lock, ShieldCheck } from "lucide-react";
...
import { CURRENT_BATCH_INFO } from "@/data/current-batch";
...
const initialInCampData: ChecklistSection[] = [
  ...
  { id: "current-batch", title: "🛡️ Current Batch Checks", icon: <ShieldCheck size={18} />, items: [
    { id: "batch-nerd", label: "NERD Verification", description: "For 2026 Batch B Stream II: complete your NERD verification before mobilisation.", completed: false, priority: "high" },
    { id: "batch-graduation-date", label: "Check Graduation Date", description: "Make sure the graduation date on your call-up letter matches your statement of result before reporting to camp.", completed: false, priority: "high" },
    { id: "batch-travel", label: "Plan Safe Daylight Travel", description: "For the current Batch B Stream II mobilisation, plan your journey to camp during daylight and follow current travel-safety guidance.", completed: false, priority: "high" },
  ] },
...
export function ClearanceChecklist() {
  ...
  return <div className="px-4 py-6 pb-24 animate-fade-in">
    ...
    <div className="bg-primary/5 border border-primary/15 rounded-2xl p-4 mb-6">
      <div className="flex items-start gap-3"><div className="w-10 h-10 rounded-xl bg-primary/10 flex items-center justify-center shrink-0"><ShieldCheck size={20} className="text-primary" /></div><div className="min-w-0"><div className="flex items-center justify-between gap-3"><div><p className="text-xs uppercase tracking-wide font-semibold text-primary">Current batch</p><p className="font-bold text-foreground">{CURRENT_BATCH_INFO.batch} • {CURRENT_BATCH_INFO.stream}</p></div><span className="text-[10px] font-medium px-2 py-1 rounded-full bg-primary/10 text-primary whitespace-nowrap">Updated Aug 21</span></div><p className="text-sm text-muted-foreground mt-2">Orientation: Aug 5–25, 2026</p><ul className="mt-3 space-y-2">{CURRENT_BATCH_INFO.notes.map((note) => <li key={note} className="text-xs text-muted-foreground flex gap-2"><span className="text-primary mt-0.5">•</span><span>{note}</span></li>)}</ul></div></div>
    </div>
